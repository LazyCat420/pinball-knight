...
def main() -> int:
    ...
    class _NoCacheHandler(http.server.SimpleHTTPRequestHandler):
        """Serve the page, never a browser's memory of it.

        This viewer rebuilds before serving, so its whole purpose is showing
        edits immediately. `SimpleHTTPRequestHandler` sends `Last-Modified`
        and no `Cache-Control`, which lets a browser reuse its copy on
        heuristic freshness without asking — the exact failure that made a
        deployed chapter appear missing on 2026-08-07. A local viewer that
        shows yesterday's page is worse than no viewer.
        """
    ...
    handler = functools.partial(_NoCacheHandler, directory=str(DOCS))
