...
fn legacy_pk_base(root: &Path) -> PathBuf {
    let direct = root.join("ThreeJS/src/game/pinball-knight");
    if direct.exists() {
        return direct;
    }
    if let Some(parent) = root.parent() {
        let parent_direct = parent.join("ThreeJS/src/game/pinball-knight");
        if parent_direct.exists() {
            return parent_direct;
        }
    }
    root.join("legacy/src/game/pinball-knight")
}

fn legacy_src_base(root: &Path) -> PathBuf {
    let direct = root.join("ThreeJS/src");
    if direct.exists() {
        return direct;
    }
    if let Some(parent) = root.parent() {
        let parent_direct = parent.join("ThreeJS/src");
        if parent_direct.exists() {
            return parent_direct;
        }
    }
    root.join("legacy/src")
}

/// Every non-test `.ts` in the PK tree, with its line count.
fn scan_legacy(root: &Path) -> BTreeMap<String, usize> {
    let base = legacy_pk_base(root);
...
fn scan_siblings(root: &Path) -> BTreeMap<String, usize> {
    ...
    for f in files {
        ...
        let rel_to_src = f
            ...
            .to_string_lossy()
            .to_string();
        if rel_to_src.ends_with(".test.ts") || rel_to_src.ends_with(".d.ts") {
            continue;
        }
        let rel = format!("legacy/src/{rel_to_src}");
        let n = std::fs::read_to_string(&f)
            .map(|s| s.lines().count())
            .unwrap_or(0);
        out.insert(rel, n);
...
fn legacy_code_lines(root: &Path) -> BTreeMap<String, usize> {
    let mut out = BTreeMap::new();
    let pk_base = legacy_pk_base(root);
    let mut files = Vec::new();
...
pub fn legacy_abs(root: &Path, path: &str) -> Option<PathBuf> {
    ...
    let pk_base = legacy_pk_base(root);
    let pk = pk_base.join(path);
    if pk.is_file() {
        return Some(pk);
    }
    if let Some(stripped) = path.strip_prefix("legacy/src/") {
        let src_base = legacy_src_base(root);
        let src = src_base.join(stripped);
        if src.is_file() {
            return Some(src);
        }
    }
