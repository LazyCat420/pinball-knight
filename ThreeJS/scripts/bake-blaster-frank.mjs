/** Run with Vite serving and a Chromium CDP endpoint. Produces the three Blaster Frank atlases. */
...
const tab = await (await fetch(`${cdp}/json/new?${encodeURIComponent(`${origin}/scripts/blaster-frank-bake.html`)}`, { method: 'PUT' })).json();
...
ws.on('message', (raw) => {
  ...
  if (m.id) {
    ...
    console.error(m.params.exceptionDetails);
  }
});
...
try {
  ...
  const baked = await send('Runtime.evaluate', { expression: `window.bakeBlasterFrankSheets()`, awaitPromise: true, returnByValue: true });
  ...
  for (const { dir, png, manifest } of baked.result.value) {
    ...
    manifest.hash = createHash('sha256').update(bytes).digest('hex').slice(0, 12);
