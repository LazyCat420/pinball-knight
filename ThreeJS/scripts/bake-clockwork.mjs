/** Run with Vite serving and a Chromium CDP endpoint. Produces the three gameplay atlases. */
import WebSocket from 'ws';
import fs from 'node:fs/promises';
import { createHash } from 'node:crypto';
const cdp=process.env.CDP_URL || 'http://localhost:9345';
const origin=process.env.PK_URL || 'http://localhost:5174';
const tab=await (await fetch(`${cdp}/json/new?${encodeURIComponent(`${origin}/scripts/clockwork-bake.html`)}`,{method:'PUT'})).json();
...
try {
 ...
 for(const emote of ['bowling','football','baseball']) {
 const baked=await send('Runtime.evaluate',{expression:`window.bakeClockworkSheets('${emote}')`,awaitPromise:true,returnByValue:true});
