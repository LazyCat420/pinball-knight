/** Run with Vite serving and a Chromium CDP endpoint. Produces the three milkshake atlases. */
...
const tab = await (await fetch(`${cdp}/json/new?${encodeURIComponent(`${origin}/scripts/milkshake-bake.html`)}`, { method: 'PUT' })).json();
...
try {
  ...
  const baked = await send('Runtime.evaluate', { expression: `window.bakeMilkshakeSheets()`, awaitPromise: true, returnByValue: true });
