/** Run with Vite serving and a Chromium CDP endpoint. Produces the three Tilt Titan atlases. */
...
const tab = await (await fetch(`${cdp}/json/new?${encodeURIComponent(`${origin}/scripts/pinball-boss-bake.html`)}`, { method: 'PUT' })).json();
...
try {
  ...
  const baked = await send('Runtime.evaluate', { expression: `window.bakePinballBossSheets()`, awaitPromise: true, returnByValue: true });
