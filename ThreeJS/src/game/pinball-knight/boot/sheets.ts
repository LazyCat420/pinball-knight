 ...
 */
const BACKFILL: SheetKey[] = ["ghost", "chomper", "jester", "croaker", "brute", "slime", "bat", "rotortail", "golem", "magnet", "spitter", "webspinner", "stiltneck", "fish_feet", "burger", "fries", "milkshake", "crawling_hand", "sumo_ninja", "zippo", "hotdog", "ketchup", "mustard", "don_quixote"];
...
export const IMPORTED_ART: Partial<Record<SheetKey, string>> = {
  ...
  zombie_lurcher: "zombie_lurcher",
  zombie_hulk: "zombie_hulk",
  zombie_midget: "zombie_midget",
  zombie_crawler: "zombie_crawler",
  zombie_flailer: "zombie_flailer",
  zombie_hobbler: "zombie_hobbler",
  zombie_plague: "zombie_plague",
  zombie_armored: "zombie_armored",
  zombie_bloated: "zombie_bloated",
  zombie_frenzy: "zombie_frenzy",
  zombie_frost: "zombie_frost",
  zombie_charred: "zombie_charred",
  zombie_screamer: "zombie_screamer",
  zombie_clutcher: "zombie_clutcher",
  zombie_gravedigger: "zombie_gravedigger",
  zombie_mummy: "zombie_mummy",
  zombie_herald: "zombie_herald",
  zombie_abomination: "zombie_abomination",
};
...
export function keysForFloor(level: number): SheetKey[] {
  const keys: SheetKey[] = ["zombie", "zombie_shambler", "zombie_lurcher", guardianFor(level).art.sheetKey];
  if (level >= 1) keys.push("goblin", "spider", "sporeling", "hound", "pin");
  if (level >= 2) keys.push("zombie_runner", "zombie_midget", "zombie_hobbler", "zombie_plague", "chomper", "croaker", "fish_feet", "jester", "ghost", "platypus", "espresso", "gnome", "cigarette", "toucan", "crawling_hand", "zippo", "clam", "crab", "gas_can", "hamster_ball", "pit_peeper", "dumpster_dan", "corvid_bomber", "gull_bomber", "toxic_slime", "highway_patrol", "hotdog", "ketchup", "mustard");
  if (level >= 3) keys.push("zombie_crawler", "zombie_flailer", "zombie_armored", "zombie_bloated", "zombie_frenzy", "zombie_frost", "don_quixote");
  ...
  if (level >= 4) keys.push("zombie_hulk", "zombie_charred", "zombie_screamer", "zombie_clutcher", "webspinner", "stiltneck", "spitter", "necromancer", "warden", "crystalback", "sumo_ninja", "shark_trapper", "lionfish_mob", "pufferfish_mob", "swordfish_mob", "ascii_human", "computer_screen", "giant_ascii_human", "void_slime", "robo_cop");
  if (level >= 5) keys.push("zombie_gravedigger", "zombie_mummy", "zombie_herald", "zombie_abomination", "reaper", "archivist", "broodmother", "dragon", "trex", "jade_buddha", "six_armed_god", "cerberus", "pinball_boss", "medusa", "dracula", "spinning_top", "octopus_gunner", "anglerfish_mob", "dragon_snake_head", "dragon_snake_body", "dragon_snake_tail", "doppelganger");
  return [...new Set(keys)];
}
...
export function rebuild(key: SheetKey): void {
  ...
  if (key === "zombie") {
    ...
    state.zombieTypeSheets.shambler = sheet;
  } else if (key.startsWith("zombie_")) {
    const ztype = key.replace(/^zombie_/, "");
    state.zombieTypeSheets[ztype] = sheet;
  }
  ...
  for (const z of state.zombies) {
    if (z.kind === "zombie") {
      const myKey = `zombie_${z.ztype ?? "shambler"}`;
      if (key !== "zombie" && myKey !== key) continue;
    } else if (sheetKeyForKind(z.kind) !== key) {
      continue;
    }
