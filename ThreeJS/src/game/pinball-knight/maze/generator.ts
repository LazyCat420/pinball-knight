...
/**
 * The tile-grid substrate now lives in `engine/grid.ts` — the data structure
 * and its accessors are not maze-specific, and collision/flow-field needed
 * them without needing maze generation.
 *
 * Re-exported from this module because the whole game imports them from here.
 */
export {
  T_WALL,
  T_FLOOR,
...
export type { Grid, TilePos } from "../engine/grid";

// Local bindings: the generator below uses these as well as re-exporting them.
import { T_WALL, T_FLOOR, T_CRACKED, at, setTile } from "../engine/grid";
import type { Grid, TilePos } from "../engine/grid";
