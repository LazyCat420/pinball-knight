...
const RAW_IMG = "/home/lazycat/.gemini/antigravity-ide/brain/20b39876-27c7-46c8-a982-0cf7dd782234/ascii_human_sheet_1789085253760.jpg";
const ASCII_HUMAN_DIR = join(SOURCES, "ascii-human-2026-09-10");
const ALT_DIR = join(ASCII_HUMAN_DIR, "alt-takes");
const MASTER_SRC = join(ALT_DIR, "ascii_human_sheet_1789085253760.jpg");

async function run() {
  console.log("👤 Preparing ASCII Binary Human Monster Sprite Sheet...");
  ...
  writeFileSync(
    readme,
    `# ASCII Binary Human Monster Sprite Sheet Archive

- **Date**: 2026-09-10
- **Subject**: ASCII Binary Human monster (\`ascii_human\`), humanoid silhouette made entirely of glowing green 0s and 1s digits. Emerges from CRT monitors, punches the player, and infects other monsters by converting them into ASCII clones.
- **Primary Source**: \`src/game/pinball-knight/tools/sprite-forge/sources/ascii-human-2026-09-10/ascii_human-S.png\`
- **Layout**: 4 columns × 4 rows (16 frames, 1024×1024, 256×256 per cell)
  - Row 0 (0..3): \`idle\` (4 frames: human binary silhouette breathing and shifting numbers)
  - Row 1 (4..7): \`walk\` (4 frames: digital humanoid walking march)
  - Row 2 (8..11): \`attack\` (4 frames: punch forward with glowing binary fists)
  - Row 3 (12..15): \`death\` (4 frames: collapsing de-rez dissolve into falling 0s and 1s)
- **Chroma Background**: \`#FF00FF\` bright magenta
- **Takes Archive**:
  - \`alt-takes/ascii_human_sheet_1789085253760.jpg\` (Master Take)
`,
  );
  ...
  const destSource = join(ASCII_HUMAN_DIR, "ascii_human-S.png");
  ...
  const sidecar = {
    ...
    notes: "ASCII binary humanoid composed of glowing green 0s and 1s",
    grid: [4, 4],
    rows: ["idle", "walk", "attack", "death"],
    rects: rectsByRow,
  };

  const inboxJson = join(INBOX, "ascii_human-S.json");
  ...
  const publicManifest = {
    name: "ascii_human",
    dir: "S",
    image: "/sprites/ascii_human-S.png",
    source: [w, h],
    hash: sha,
    rows,
  };

  const publicJson = join(PUBLIC_SPRITES, "ascii_human-S.json");
  writeFileSync(publicJson, JSON.stringify(publicManifest, null, 1));
  console.log(`Saved public manifest to ${publicJson}`);
  console.log("✅ ASCII Binary Human sprite prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep ascii human sprite sheet:", err);
  process.exit(1);
});
