...
const MASTER_SRC = join(SOURCES, "bloater-2026-09-06", "alt-takes", "bloater_garbage_monster_sheet_1788675076508.jpg");

async function run() {
  console.log("🗑️ Preparing Bloater (Garbage Monster) Sprite Sheet (Nano Banana + Sprite Forge)...");
  ...
  const destSource = join(SOURCES, "bloater-2026-09-06", "bloater-S.png");
  ...
  const inboxPng = join(INBOX, "bloater-S.png");
  ...
  const manifest = {
    ...
    notes: "Bloater garbage monster exploding into flaming debris and magma crater",
    grid: [4, 4],
    rows: rows,
    rects: rects,
  };

  const inboxJson = join(INBOX, "bloater-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ Bloater sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep bloater sprite sheet:", err);
  process.exit(1);
});
