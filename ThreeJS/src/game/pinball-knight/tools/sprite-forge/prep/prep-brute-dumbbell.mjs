...
const RAW_IMG = "/home/lazycat/.gemini/antigravity-ide/brain/7602ad38-13aa-4260-841b-78c2dd993ee3/brute_dumbbell_sheet_1789676723675.jpg";
const BRUTE_DIR = join(SOURCES, "brute-2026-09-17");
const ALT_DIR = join(BRUTE_DIR, "alt-takes");
const MASTER_SRC = join(ALT_DIR, "brute_dumbbell_sheet_1789676723675.jpg");

async function run() {
  console.log("🏋️ Preparing Brute (Gym Behemoth & Dumbbells) Sprite Sheet...");
  ...
  console.log(`Loaded master image dimensions: ${w}x${h}`);
  ...
  function cleanSheet(srcImg) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      // Protect Brute features:
      // Dark iron dumbbells, shadows, boots, wrist wraps
      const isDarkIronOrShadow = r < 75 && g < 75 && b < 75;
      // Brown leather belt, buckle, pants
      const isBrownOrLeather = (r > 60 && g < r && b < g) || (r > 100 && g > 70 && b < 60);
      // Orc skin (has red/blue tones mixed in, not pure green)
      const isOrcSkin = (r > 40 && g > 50 && b > 20 && (r > 60 || b > 40));
      // Highlights / teeth / eye whites / belt buckle gold
      const isHighlight = (r > 150 && g > 150 && b > 120);

      // Pure background chroma green: intense green with low red and low blue
      const isChromaGreen = !isDarkIronOrShadow && !isBrownOrLeather && !isOrcSkin && !isHighlight && (
        (g > 180 && r < 60 && b < 60) ||
        (g > 200 && (r + b) < 130)
      );

      // Outer border frame (3px)
      const isOuterBorder = px < 2 || px >= w - 2 || py < 2 || py >= h - 2;

      if (isChromaGreen || isOuterBorder) {
        d[i] = 0;
        d[i + 1] = 255;
        d[i + 2] = 0;
  ...
  const destSource = join(BRUTE_DIR, "brute-S.png");
  ...
  const inboxPng = join(INBOX, "brute-S.png");
  ...
  const manifest = {
    ...
    notes: "Gym behemoth orc brute wielding cast-iron dumbbells with ranged heave, wall ricochet, and ground smash mechanics",
    grid: [4, 4],
    rows: rows,
    rects: rects,
    matte: {
  ...
  const inboxJson = join(INBOX, "brute-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ Brute dumbbell sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep brute sprite sheet:", err);
  process.exit(1);
});
