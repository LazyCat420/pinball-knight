  ...
  function cleanSheet(srcImg) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      if (isMagenta || isGridLine || isOuterBorder) {
        ...
        // Balance burger colors with dungeon palette
        const luma = 0.299 * r + 0.587 * g + 0.114 * b;
        d[i] = Math.round(Math.min(255, Math.max(0, luma + (r - luma) * 0.82)));
        d[i + 1] = Math.round(Math.min(255, Math.max(0, luma + (g - luma) * 0.82)));
        d[i + 2] = Math.round(Math.min(255, Math.max(0, luma + (b - luma) * 0.82)));
      }
    }
