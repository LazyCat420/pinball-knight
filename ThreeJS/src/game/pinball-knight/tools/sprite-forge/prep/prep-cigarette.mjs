...
const RAW_IMG = "/home/lazycat/.gemini/antigravity-ide/brain/a200ad12-411c-4af5-bf7d-173a4d6b96f9/cigarette_cartoon_sheet_1788749152171.jpg";
const CIG_DIR = join(SOURCES, "cigarette-2026-09-06");
const ALT_DIR = join(CIG_DIR, "alt-takes");
const MASTER_SRC = join(ALT_DIR, "cigarette_master.jpg");

async function run() {
  console.log("🚬 Preparing 1950s Cartoon Walking Cigarette Sprite Sheet...");
  ...
  const destSource = join(CIG_DIR, "cigarette-S.png");
  ...
  const inboxPng = join(INBOX, "cigarette-S.png");
  ...
  const manifest = {
    ...
    notes: "1950s Rubberhose Walking Cigarette with burning cherry ember attack and stubbed-out ash death",
    grid: [4, 4],
    rows: rows,
    rects: rects,
    matte: {
  ...
  const inboxJson = join(INBOX, "cigarette-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ 1950s Cartoon Walking Cigarette sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep cigarette sprite sheet:", err);
  process.exit(1);
});
