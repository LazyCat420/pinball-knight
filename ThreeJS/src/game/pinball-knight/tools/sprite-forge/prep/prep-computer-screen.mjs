...
const RAW_IMG = "/home/lazycat/.gemini/antigravity-ide/brain/20b39876-27c7-46c8-a982-0cf7dd782234/computer_screen_sheet_1789085265927.jpg";
const COMPUTER_DIR = join(SOURCES, "computer-screen-2026-09-10");
const ALT_DIR = join(COMPUTER_DIR, "alt-takes");
const MASTER_SRC = join(ALT_DIR, "computer_screen_sheet_1789085265927.jpg");

async function run() {
  console.log("🖥️ Preparing Computer Screen CRT Spawner Sprite Sheet...");
  ...
  writeFileSync(
    readme,
    `# Computer Screen Spawner Sprite Sheet Archive

- **Date**: 2026-09-10
- **Subject**: Computer Screen spawner (\`computer_screen\`), retro 1980s CRT monitor on desk terminal. Scattered in Level 4 mazes, flashes and emits ASCII Binary Humans until smashed by the player.
- **Primary Source**: \`src/game/pinball-knight/tools/sprite-forge/sources/computer-screen-2026-09-10/computer_screen-S.png\`
- **Layout**: 4 columns × 4 rows (16 frames, 1024×1024, 256×256 per cell)
  - Row 0 (0..3): \`idle\` (4 frames: CRT scanlines, green terminal text glowing)
  - Row 1 (4..7): \`walk\` (4 frames: screen surges with matrix code, spawning pulse)
  - Row 2 (8..11): \`attack\` (4 frames: electrical static flare, screen distortion)
  - Row 3 (12..15): \`death\` (4 frames: glass implosion, spark explosion, smoking chassis)
- **Chroma Background**: \`#FF00FF\` bright magenta
- **Takes Archive**:
  - \`alt-takes/computer_screen_sheet_1789085265927.jpg\` (Master Take)
`,
  );
  ...
  const destSource = join(COMPUTER_DIR, "computer_screen-S.png");
  ...
  // Inbox sidecar
  const sidecar = {
    sheet: "computer_screen-S",
    author: "LazyCat420 & Nano Banana",
    notes: "Retro CRT computer terminal spawner that manifests ASCII binary humans",
    grid: [4, 4],
    rows: ["idle", "walk", "attack", "death"],
    rects: rectsByRow,
  };

  const inboxJson = join(INBOX, "computer_screen-S.json");
  ...
  const publicManifest = {
    ...
    dir: "S",
    image: "/sprites/computer_screen-S.png",
    source: [w, h],
    hash: sha,
    rows,
  };

  const publicJson = join(PUBLIC_SPRITES, "computer_screen-S.json");
  writeFileSync(publicJson, JSON.stringify(publicManifest, null, 1));
  console.log(`Saved public manifest to ${publicJson}`);
  console.log("✅ Computer Screen spawner sprite prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep computer screen sprite sheet:", err);
  process.exit(1);
});
