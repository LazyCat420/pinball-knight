...
const RAW_IMG = "/home/lazycat/.gemini/antigravity-ide/brain/878e1b68-7679-4a65-bf9d-283c58c7185a/crab_knife_sheet_1788827345041.jpg";
const CRAB_DIR = join(SOURCES, "crab-2026-09-07");
const ALT_DIR = join(CRAB_DIR, "alt-takes");
const MASTER_SRC = join(ALT_DIR, "crab_knife_sheet_1788827345041.jpg");

async function run() {
  console.log("🦀 Preparing Dapper Knife Crab (Top Hat & Monocle) Sprite Sheet...");
  ...
  writeFileSync(
    readme,
    `# Dapper Knife Crab Monster Sprite Sheet Archive

- **Date**: 2026-09-07
- **Subject**: Dapper Knife Crab monster (\`crab\`), a Victorian gentleman crustacean sporting a black top hat, golden monocle, and sharp steel knife blades in place of pincers.
- **Primary Source**: \`src/game/pinball-knight/tools/sprite-forge/sources/crab-2026-09-07/crab-S.png\`
- **Layout**: 4 columns × 4 rows (16 frames, 1024×1024, 256×256 per cell)
  - Row 0 (0..3): \`idle\` (4 frames: aristocratic poise with tilted top hat, monocle glints, knife arms held high)
  - Row 1 (4..7): \`walk\` (4 frames: sideways crab scuttle with top hat swaying and knife blades swinging)
  - Row 2 (8..11): \`attack\` (4 frames: overhead dual-knife windup telegraph into a scissor cross-slash with bright steel arc trail)
  - Row 3 (12..15): \`death\` (4 frames: shell cracks, top hat & monocle fly off, blades detach and clatter down)
- **Chroma Background**: \`#00FF00\` bright green
- **Takes Archive**:
  - \`alt-takes/crab_knife_sheet_1788827345041.jpg\` (Take 1 - Master: Dapper Knife Crab)
`,
  );
  ...
  function cleanSheet(srcImg) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      // Protect crab features:
      // Top hat / dark shadows
      const isHat = r < 55 && g < 55 && b < 55;
      // Red/orange shell or tan underbelly
      const isShell = (r > 90 && r > g * 1.1) || (r > 90 && g > 65 && b > 40 && Math.abs(r - g) < 70);
      // Gold monocle / trim
      const isMonocle = r > 150 && g > 120 && b < 100;
      // Steel knife blades / white slash arcs
      const isBlade = (r > 140 && g > 140 && b > 140 && Math.abs(r - g) < 40 && Math.abs(g - b) < 40);

      // Green chroma key check (high green, noticeably lower red and blue)
      const isGreen = !isHat && !isShell && !isMonocle && !isBlade && (
  ...
  const destSource = join(CRAB_DIR, "crab-S.png");
  ...
  const inboxPng = join(INBOX, "crab-S.png");
  ...
  const manifest = {
    ...
    notes: "Dapper Knife Crab monster with top hat and monocle, attacks by slashing with blade arms",
    grid: [4, 4],
    rows: rows,
    rects: rects,
    matte: {
  ...
  const inboxJson = join(INBOX, "crab-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ Dapper Knife Crab sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep crab sprite sheet:", err);
  process.exit(1);
});
