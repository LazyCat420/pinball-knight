...
`,
  ...
  // Chroma key helper for #FF00FF magenta + edge cleanup
  ...
  const destSource = join(BUDDHA_DIR, "jade_buddha-S.png");
  ...
  const inboxPng = join(INBOX, "jade_buddha-S.png");
  ...
  const manifest = {
    sheet: "jade_buddha-S",
    author: "LazyCat420",
    notes: "The Jade Buddha Boss seated on golden lotus with Chinese war fan",
    grid: [4, 4],
    rows: rows,
    rects: rects,
    matte: {
  ...
  const inboxJson = join(INBOX, "jade_buddha-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ The Jade Buddha sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep jade buddha sprite sheet:", err);
  process.exit(1);
});
