...
const MASTER_SRC = join(SOURCES, "warden-2026-09-06", "alt-takes", "warden_cop_guard_sheet_1788676686705.jpg");

async function run() {
  console.log("👮 Preparing Warden (Wolfenstein Cop Guard) Sprite Sheet (Nano Banana + Sprite Forge)...");
  ...
  const destSource = join(SOURCES, "warden-2026-09-06", "warden-S.png");
  ...
  const inboxPng = join(INBOX, "warden-S.png");
  ...
  const manifest = {
    ...
    notes: "Warden cop guard (Wolfenstein 3D hybrid) firing ricocheting service bullets",
    grid: [4, 4],
    rows: rows,
    rects: rects,
  };

  const inboxJson = join(INBOX, "warden-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ Warden cop guard sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep warden sprite sheet:", err);
  process.exit(1);
});
