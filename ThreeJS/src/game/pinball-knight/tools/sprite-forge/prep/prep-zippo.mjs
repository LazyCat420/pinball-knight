...
const RAW_IMG = "/home/lazycat/.gemini/antigravity-ide/brain/a5065001-71bf-4a83-a63d-388b09e8dc16/zippo_lighter_sheet_1788769694702.jpg";
const ZIPPO_DIR = join(SOURCES, "zippo-2026-09-07");
const ALT_DIR = join(ZIPPO_DIR, "alt-takes");
const MASTER_SRC = join(ALT_DIR, "zippo_lighter_sheet_1788769694702.jpg");

async function run() {
  console.log("🔥 Preparing Pyro Zippo (1960s Cartoon Zippo Lighter) Sprite Sheet...");
  ...
  function cleanSheet(srcImg) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      // Flame protection: yellow/orange/red pixels (high R, mid/high G, low B)
      const isFlame = r > 160 && (r > b * 1.5);
      // Chrome lighter protection: near-gray/white/silver (r, g, b are close)
      const isChrome = Math.abs(r - g) < 25 && Math.abs(g - b) < 25 && (r + g + b) > 120;

      // Green chroma key check (high green, noticeably lower red and blue)
      const isGreen = !isFlame && !isChrome && (
  ...
  const destSource = join(ZIPPO_DIR, "zippo-S.png");
  ...
  const inboxPng = join(INBOX, "zippo-S.png");
  ...
  const manifest = {
    ...
    notes: "1960s Cartoon Zippo Lighter monster with alcohol bottle chug and flame breath attack",
    grid: [4, 4],
    rows: rows,
    rects: rects,
    matte: {
  ...
  const inboxJson = join(INBOX, "zippo-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ Pyro Zippo sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep zippo sprite sheet:", err);
  process.exit(1);
});
