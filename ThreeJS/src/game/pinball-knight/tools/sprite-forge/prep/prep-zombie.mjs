...
const RAW_IMG = "/home/lazycat/.gemini/antigravity-ide/brain/50a9bb87-0312-402e-9b4b-5425c382a7b8/zombie_master_sheet_1789363406770.jpg";
const ZOMBIE_DIR = join(SOURCES, "zombie-2026-09-14");
const ALT_DIR = join(ZOMBIE_DIR, "alt-takes");
const MASTER_SRC = join(ALT_DIR, "zombie_master_take1.jpg");

async function run() {
  console.log("🧟 Preparing Zombie South-Facing Master Sprite Sheet...");
  ...
  writeFileSync(
    readme,
    `# Undead Zombie Monster Sprite Sheet Archive

- **Date**: 2026-09-14
- **Subject**: Baseline Undead Zombie (\`zombie\`), 16-bit retro arcade pixel art on pure chroma green #00FF00.
- **Primary Source**: \`src/game/pinball-knight/tools/sprite-forge/sources/zombie-2026-09-14/zombie-S.png\`
- **Layout**: 4 columns × 4 rows (16 frames, 1024×1024, 256×256 per cell)
  - Row 0 (0..3): \`idle\` (4 frames: shuffling, menacing breathing stance, head twitch)
  - Row 1 (4..7): \`walk\` (4 frames: dragging forward shambling gait, lunging step, arms outstretched)
  - Row 2 (8..11): \`attack\` (4 frames: double-arm ferocious claw swipe and vicious bite)
  - Row 3 (12..15): \`death\` (4 frames: stumble backward, collapse to knees, dissolve/fall flat to the floor)
- **Chroma Background**: \`#00FF00\` pure green
- **Takes Archive**:
  - \`alt-takes/zombie_master_take1.jpg\` (Take 1 - Master: Undead Zombie generated via Nano Banana)
`,
  );
  ...
  function cleanSheet(srcImg) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      // Protect Zombie features:
      // Dark clothing, black outlines, charcoal pants
      const isDark = r < 75 && g < 75 && b < 75;
      // Purple shirt / shroud
      const isPurple = (r > 60 && b > 60 && g < 75) || (r > 90 && b > 90 && g < 110);
      // Yellow glowing eyes and teeth
      const isYellow = r > 160 && g > 150 && b < 80;
      // Rot-green necrotic skin (green with substantial red/blue or lower brightness than flat chroma)
      const isRotGreen = (g > 60 && g < 210 && (r > 40 || b > 20)) || (g > 90 && r > 50);
      // Exposed bone/ribs/teeth (pale ivory)
      const isBone = r > 140 && g > 140 && b > 100;
      // Red blood/gore
      const isRed = r > 120 && g < 60 && b < 60;

      const isSpritePixel = isDark || isPurple || isYellow || isRotGreen || isBone || isRed;

      // Pure background chroma green check: high green with low red and low blue
      const isChromaGreen = !isSpritePixel && (
        (g > 160 && r < 90 && b < 90) ||
        (g > 200 && (r + b) < 180)
      );
  ...
  const destSource = join(ZOMBIE_DIR, "zombie-S.png");
  ...
  const inboxPng = join(INBOX, "zombie-S.png");
  ...
  const manifest = {
    ...
    notes: "Classic undead rot-green zombie monster facing South in ragged burial garments (idle, walk, attack, death)",
    grid: [4, 4],
    rows: rows,
    rects: rects,
    matte: {
  ...
  const inboxJson = join(INBOX, "zombie-S.json");
  writeFileSync(inboxJson, JSON.stringify(manifest, null, 2));
  console.log(`Saved inbox manifest to: ${inboxJson}`);
  console.log("✅ Zombie South-facing sprite sheet prep complete!");
}

run().catch((err) => {
  console.error("Failed to prep zombie sprite sheet:", err);
  process.exit(1);
});
