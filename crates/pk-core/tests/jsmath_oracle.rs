...
fn every_swept_primitive_matches_the_runtime() {
    ...
    // sin/cos: 5 ranges each. exp: 8. log: 5. sqrt, atan: 1 each. tan: 4.
    assert_eq!(checked, 29, "a sweep stopped being checked");
    assert!(
        failed.is_empty(),
...
fn the_rejected_candidates_still_disagree() {
    ...
    // cos 5 + sin 5 + exp 8 + log 5. (`tan`/`atan2` are controlled separately
    // below — for them `libm` must AGREE, which is the opposite assertion.)
