# Summary

[Overview](overview.md)

# Game

- [Architecture decisions](game/architecture.md)
- [Milestones & sizing](game/milestones.md)

# Sprite-Forge & Art

- [Art pipelines](art/pipelines.md)
- [Bake pipeline & manifest schema](art/bake.md)

# Port Status

- [Status board](status/board.md)
- [**The route to 1:1** (read this first — what is left, in what order)](status/one-to-one-route.md)
- [Recorded baselines (every number, generated — never hand-edited)](status/baselines.md)
- [Port plan v3 (the remainder off the ledger, and the queue)](status/port-plan-v3.md)
- [The 1:1 plan (what "converted" means, and what is left)](status/one-to-one.md)
- [The tavern, 1:1 (station by station)](status/tavern-one-to-one.md)
- [Port checklist (phases P0–P9)](status/port-checklist.md)
- [Completion plan (the route to cutover)](status/completion-plan.md)
- [Build-out queue (what gets built next, in order)](status/build-out.md)
- [Handoff (state of the baton)](status/handoff.md)
- [Incidents](status/incidents.md)

# Reference

- [Dev environment (WSL2, shared box)](reference/dev-env.md)
- [Legacy tree map](reference/legacy.md)
- [The gate scripts (scoped-gate, full-suite, the lease)](reference/gate-scripts.md)
