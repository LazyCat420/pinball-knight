...
| I-7 | **`pk-check` has only ever been run against a DEBUG build, and the RELEASE build fails two of its gates**, reproduced twice. The click-skip path passes and every tavern gate after it passes, so the hub does build — the failure is at the natural-handoff *sampling edge*. Also: the sim-rate gate reads 66 Hz on debug and **72 Hz on release** for a fixed-step sim | open — **and it is the gate the whole project treats as the definition of green** |
