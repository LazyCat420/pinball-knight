...
| ~~V-2~~ | ✅ **DONE 2026-08-11.** The bucket key gained a `surface`, and each bucket takes a material tinted by that surface's own hex — the merged-mesh equivalent of the oracle's per-INSTANCE `setColorAt(k, surf.hex)`. Stone is an explicit WHITE, not an omission; the tint MULTIPLIES the bake so the courses and normal map still read. Moss and the knee-high rims are tinted too (the rim is the piece the ball hits most) | `pk-ab-dungeon --level 3 --seed 1`: mean 30.2 → **30.1**, over32 33.8% → **33.6%** — the V-1 baseline reproduced exactly, so the rig is stable. Port-frame diff: **21,411 px changed, max delta 223**, landing on wall faces in tile-shaped blocks with the floor untouched. Buckets 3 → 12 (L1/L3), 3 → 9 (L5). ⚠️ A test here asserted ≥600 planned tiles on L3 and got **213** — the renderer culls unexposed walls and skips shaped tiles; re-derived over the JSON with those two filters it is 213 exactly, so the test now asserts the renderer's number rather than the file's |
...
| ~~**I-1**~~ | ✅ shipped `54e506e`, **repaired 2026-08-11** — `scripts/pk-ab-intro.mjs` | Stage 2's gate | sheets at `.checks/ab-intro-*.png`. It waited a fixed 1.2 s and shot whatever was there, so on a loaded box the ORACLE was photographed earlier in the phase than the port while both captions said "frozen" (shatter 88.9 then 58.4 across two runs; sweep's oracle luma moved 23 → 40). It now POLLS `__abFrozen()` and **throws rather than shooting** if the clock never arrives. **Every intro number taken before this is partly the harness** — the honest baseline is in [handoff](handoff.md) |
